 jQuery(function ($) { 
	...
	function fillInAddress() {
		...
		if(place.address_components != null && place.address_components.length > 0 ){
			...
			for (var i = 0; i < place.address_components.length; i++) {
			      for (var j = 0; j < place.address_components[i].types.length; j++) {
			        ...
			        if (place.address_components[i].types[j] == "sublocality_level_1" || place.address_components[i].types[j] =="neighborhood") {
		...
		$('#country').val(country);
		$('#zipcode').val(zip);
		$('#address2').val(landmark);
		//alert($('#address1').val()  + $('#city').val() + $('#state').val() + $('#zipcode').val() + $('#country').val());
	}
 });